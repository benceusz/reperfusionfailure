...
def run_ct_coreg(DATA_DIR):
    ...
    MASK_FILES = ["mask_core", "mask_penumbra", "mask_penumbra_bl", "mask_penumbra_v1"]
    DWI_FILES = []
    NIFTI_FILES = []
    BOLD_FILES = []
...
if __name__ == '__main__':
    ...
    for i_patient in subdirs(PROJECT_DIR):
        print("processing patient: %s" % i_patient)
        print("Mni path %s"%PATH_MNI_BRAINMASK)
        for i_dir in subdirs(os.path.join(PROJECT_DIR, i_patient)):
